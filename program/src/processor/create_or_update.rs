...
pub(crate) fn create_or_update<'a>(
    ...
    match args {
        ...
        CreateOrUpdateArgs::V2 {
            serialized_rule_set,
        } => create_or_update_v2(program_id, context, serialized_rule_set),
    }
}
...
fn create_or_update_v2(
    ...
    let (rule_set_version, rule_set_name, owner) = match ctx.accounts.buffer_pda_info {
        Some(account_info) => {
            ...
            let rule_set = RuleSetV2::from_bytes(data)
                .map_err(|_| RuleSetError::MessagePackDeserializationError)?;
            (rule_set.lib_version(), rule_set.name(), *rule_set.owner)
        }
        None => {
    ...
    // Make sure we know how to work with this RuleSet.
    if rule_set_version != LibVersion::V2 as u8 {
        return Err(RuleSetError::UnsupportedRuleSetVersion.into());
    }
        ...
        || is_zeroed(&ctx.accounts.rule_set_pda_info.data.borrow())
    {
        ...
        revision_map
            .rule_set_revisions
            .push(RULE_SET_SERIALIZED_HEADER_LEN + ALIGNMENT_PADDING);
        revision_map
    } else {
        // Get existing revision map and its serialized length.
        let (mut revision_map, existing_rev_map_loc) =
        ...
        // alignment required
        let alignment = existing_rev_map_loc
    ...
    // Determine size needed for PDA: next revision location (which is:
    // (RULE_SET_SERIALIZED_HEADER_LEN || existing latest revision map location))
    // + 1 byte for revision map version number
    // + length of user-pre-serialized `RuleSet`
