...
fn get_composed_rules() -> ComposedRules {
    ...
    let transfer_rule = Rule::All {
        rules: vec![
            nft_amount.clone(),
            Rule::Not {
                rule: Box::new(multi_field_program_allow_list),
            },
        ],
    };
    ...
    let delegate_rule = Rule::All {
        rules: vec![
            nft_amount.clone(),
            Rule::Not {
                rule: Box::new(delegate_program_allow_list),
            },
        ],
    };
    ...
    let advanced_delegate_rule = Rule::All {
        rules: vec![
            nft_amount,
            Rule::Not {
                rule: Box::new(advanced_delegate_program_allow_list),
            },
        ],
    };
...
async fn prog_owned_to_prog_owned() {
    ...
    // Check that error is what we expect.  Program owner was on the deny list.
    match err {
        solana_program_test::BanksClientError::TransactionError(
            TransactionError::InstructionError(_, InstructionError::Custom(error)),
        ) => {
